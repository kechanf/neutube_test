	...
	if (Locseg_Chain_Connection_Test(chain_i, chain_j, stack, 
	  ...
	  if (i > j) { // 第二次判断两节点的连接情况
	    if (ns->graph->nedge > 0) {
	      ...
	      if (edge_idx >= 0) { // 存在路径
          if (conn.mode == NEUROCOMP_CONN_LINK && ns->conn[edge_idx].info[0] == conn.info[1]) {
            conn_existed = TRUE;
          } else if (ns->conn[edge_idx].mode == NEUROCOMP_CONN_LINK && ns->conn[edge_idx].info[1] == conn.info[0]) {
            conn_existed = TRUE;
          }
          if (conn_existed == TRUE && ns->conn[edge_idx].cost > conn.cost) {
			      ...
			      int nseed, Trace_Workspace *tw, int *nchain)
{
  ...
  *nchain = 0;
  ...
  for(int temp_i = (int)(temp_soma_pos[0]) - 10; temp_i <= (int)(temp_soma_pos[0]) + 10; temp_i ++)
    for(int temp_j = (int)(temp_soma_pos[1]) - 10; temp_j <= (int)(temp_soma_pos[1]) + 10; temp_j ++)
      for(int temp_k = (int)(temp_soma_pos[2]) - 10; temp_k <= (int)(temp_soma_pos[2]) + 10; temp_k ++)
      {
        // tw->trace_mask[temp_i][temp_j][temp_k] = 1;
        double dist2soma = Geo3d_Dist(temp_i, temp_j, temp_k, 
                            temp_soma_pos[0], temp_soma_pos[1], temp_soma_pos[2]);
        if(dist2soma <= 7)
        {
          Set_Stack_Pixel(tw->trace_mask, temp_i, temp_j, temp_k, 0, 1);
        }
      }
...
#define TRACE_LOCSEG_S_RECORD_CHAIN(new_chain)				\
      ...
      if ((Locseg_Chain_Geolen(locseg_chain) >= tw->min_chain_length * 0.1) ||
          ((tw->trace_status[0] == TRACE_HIT_MARK) || 
           (tw->trace_status[1] == TRACE_HIT_MARK))) {
  ...
  tr->mask = ZERO_BIT_MASK;
  Trace_Record_Set_Direction(tr, DL_BOTHDIR);
  Local_Neuroseg *soma_seg = New_Local_Neuroseg();
  Set_Local_Neuroseg(soma_seg, 6.0, 0.0, 0.01, -TZ_PI_2, 0.0, 0.0,
                      0.0, 1.0, 
                      temp_soma_pos[0], temp_soma_pos[1], temp_soma_pos[2]);
  Locseg_Node *p = Make_Locseg_Node(Copy_Local_Neuroseg(soma_seg), tr);
  Locseg_Chain *locseg_chain = Make_Locseg_Chain(p);
  Unipointer_Queue_En(&chain_list, locseg_chain);
  (*nchain)++;
